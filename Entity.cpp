...
void Entity::Draw(std::stack<glm::mat4> mvStack, glm::mat4 projection, GLuint shaderProgram, GLfloat playerRotation)
{
	...
	rt3d::drawMesh(model, vertCount, GL_TRIANGLES);
	mvStack.pop();
	if (cull == true) {
		glCullFace(GL_BACK);
	}
...
void Entity::setPosition(glm::vec3 inPositionUpdate)
{
	position = inPositionUpdate;
}

void Entity::setRotationY(GLfloat inRotationUpdate)
{
	rotation.y = inRotationUpdate;
}

void Entity::setRotationX(GLfloat inRotationUpdate)
{
	rotation.z = inRotationUpdate;
}
